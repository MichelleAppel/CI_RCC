...
class Client:
	...
	def run(self, NO_FRAMES):
		...
		self.state = State.STOPPED
		return self.fitness, self.offroad_count, self.turn_around_count, self.negative_speed_count
	...
	def _process_server_msg(self):
		try:
			...
			if not buffer:
				...
				command, self.fitness, self.offroad_count, self.turn_around_count, self.negative_speed_count = self.driver.drive(carstate, self.offroad_count, self.turn_around_count, self.negative_speed_count)
