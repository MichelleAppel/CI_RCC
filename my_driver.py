...
def normalize(vector):
		return np.array(vector)/(np.max(vector)-np.min(vector))
...
class MyDriver(Driver):
	...
	def drive(self, carstate: State, offroad_count, turn_around_count, negative_speed_count) -> Command:
		...
		# for i, dist in enumerate(opponents):
		# 	if dist == 200.:
		# 		opponents[i] = 0.
		...
		if middle_sens == -1.:
			...
			for i, dist in enumerate(edges):
				if dist > 0:
					edges[i] = np.exp(-dist/react_dist)/19
				else:
					edges[i] = -np.exp(dist/react_dist)/19
		...
		input_vector = edges + [carstate.speed_x/max_speed] + [carstate.angle/max_angle] + \
		[carstate.distance_from_center] + [carstate.rpm/max_rpm]
